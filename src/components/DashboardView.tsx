...
export function DashboardView({
  ...
  const { data: rawOperations = [] } = useQuery(operationsQuery);
  const { data: rawDayEntries = [] } = useQuery(entriesQuery(date));
  const { data: rawAllEntries = [] } = useQuery(entriesQuery());
  const { data: rawSectors = [] } = useQuery(sectorsQuery);
  const { data: rawCatalogOps = [] } = useQuery(catalogOperationsQuery);
  const { data: esteira = [] } = useQuery(esteiraQuery);
  const { data: rawMetas = [] } = useQuery(metaProducaoQuery(date));

  /* ---- recorte por setor (dashboards de setor) ---- */
  const sectors = useMemo(
    () => (sectorId ? rawSectors.filter((x) => x.id === sectorId) : rawSectors),
    [rawSectors, sectorId],
  );
  const catalogOps = useMemo(
    () => (sectorId ? rawCatalogOps.filter((c) => c.sector_id === sectorId) : rawCatalogOps),
    [rawCatalogOps, sectorId],
  );
  const sectorCatalogIds = useMemo(() => new Set(catalogOps.map((c) => c.id)), [catalogOps]);
  const operations = useMemo(
    () =>
      sectorId
        ? rawOperations.filter(
            (o) => o.catalog_operation_id && sectorCatalogIds.has(o.catalog_operation_id),
          )
        : rawOperations,
    [rawOperations, sectorCatalogIds, sectorId],
  );
  const sectorOperationIds = useMemo(() => new Set(operations.map((o) => o.id)), [operations]);
  const dayEntries = useMemo(
    () =>
      sectorId ? rawDayEntries.filter((e) => sectorOperationIds.has(e.operation_id)) : rawDayEntries,
    [rawDayEntries, sectorOperationIds, sectorId],
  );
  const allEntries = useMemo(
    () =>
      sectorId ? rawAllEntries.filter((e) => sectorOperationIds.has(e.operation_id)) : rawAllEntries,
    [rawAllEntries, sectorOperationIds, sectorId],
  );
  const metas = useMemo(
    () => (sectorId ? rawMetas.filter((m) => m.sector_id === sectorId) : rawMetas),
    [rawMetas, sectorId],
  );
  ...
  return (
    <AppLayout title={title} subtitle={subtitle} requireAdmin={false}>
      ...
      <div className="space-y-5">
        <Card>
          <CardContent className="grid grid-cols-1 gap-3 pt-6 sm:flex sm:flex-wrap sm:items-end sm:gap-4">
            ...
            <div className="rounded-md bg-secondary sm:ml-auto px-4 py-2 text-right">
